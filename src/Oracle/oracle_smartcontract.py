           ...
           Assert(         #Checks that the subsequent conditions are met and terminates the sequence if they are not
               And(
                   ...
                   Txn.application_args.length() == Int(3)  #Only three arguments must be passed during the contract call
                                                            #[transaction_call_type, image_hash, participant_public_address]
               )
           ),
...
#Compile the approval and clear state programs to teal code
if __name__ == "__main__":
    with open("oracle_smartcontract_approval.teal", "w") as f:
        compiled = compileTeal(approval_program(), mode=Mode.Application, version=5)
        f.write(compiled)

    with open("oracle_smartcontract_clear_state.teal", "w") as f:
